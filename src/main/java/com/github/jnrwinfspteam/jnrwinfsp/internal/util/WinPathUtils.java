...
import com.github.jnrwinfspteam.jnrwinfsp.internal.lib.LibException;
...
public final class WinPathUtils {
     ...
     *
     * @return WinFsp library path
     * @throws LibException if the path can't be found
     */
    public static String getWinFspPath() {
        if (!Platform.IS_WINDOWS) {
            throw new LibException("WinFsp can only be configured on Windows");
        }

        // if path was set as a property, don't try any default value, just bail if not found
        String configuredPath = System.getProperty("jnrwinfsp.lib.path");
        if (configuredPath != null && !configuredPath.isEmpty()) {
            if (!libExists(configuredPath)) {
                throw new LibException("Configured WinFsp library (jnrwinfsp.lib.path property) " +
                        configuredPath + " can't be found");
            }
            return configuredPath;
        }

        // try extracting from the registry
        ...
        throw new LibException("Can't find WinFsp library. Please make sure that WinFsp is installed or configure " +
                "the path to dll manually using the `jnrwinfsp.lib.path` property");
    }
